...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: team, error: teamError } = await supabaseAdmin
      .from('teams')
      .select('id, association_id, name, max_players')
      .eq('id', teamId)
      .single();
    ...
    const teamName = String((team as any).name || '');
    ...
    const { data: player, error: playerError } = await supabaseAdmin
      .from('profiles')
      .select('id, role, association_id, team')
      .eq('id', playerId)
      .single();
    ...
    if (String((player as any).team || '') === teamName) {
